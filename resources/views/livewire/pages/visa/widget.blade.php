<div>
        ...
        <div class="vw-hero__inner">
            ...
            <form class="vw-card" action="{{ route('visa.search') }}" method="POST">
                ...
                <div class="vw-fields">
                    ...
                    <label class="vw-field"><span>Destination</span><select name="destination_ref" x-model="destinationRef" @change="sameCountryNotice = false" required><option value="">Where are you going?</option>@if($destinationCountries->isNotEmpty())<optgroup label="Countries">@foreach($destinationCountries as $country)<option value="country:{{ $country->id }}" :disabled="String(nationalityId) === '{{ $country->id }}'">{{ $country->name }}</option>@endforeach</optgroup>@endif @if($regionalDestinations->isNotEmpty())<optgroup label="Regional visa destinations">@foreach($regionalDestinations as $destination)<option value="region:{{ $destination->id }}">{{ $destination->name }}</option>@endforeach</optgroup>@endif</select><small x-show="sameCountryNotice" x-cloak role="status">Choose a destination different from your passport nationality.</small></label>
    ...
    <script>
        function visaWidget(){return{paxOpen:false,nationalityId:@js((string) old('nationality_id', $defaultNationalityId)),destinationRef:@js((string) old('destination_ref', '')),sameCountryNotice:false,counts:{adults:{{ old('adults',1) }},children:{{ old('children',0) }},infants:{{ old('infants',0) }}},passengerRows:[{key:'adults',label:'Adults',hint:'12+ years',min:1},{key:'children',label:'Children',hint:'2–11 years',min:0},{key:'infants',label:'Infants',hint:'Under 2 years',min:0}],handleNationalityChange(){if(this.destinationRef===`country:${this.nationalityId}`){this.destinationRef='';this.sameCountryNotice=true}else{this.sameCountryNotice=false}},totalPassengers(){return this.counts.adults+this.counts.children+this.counts.infants},travelerLabel(){const total=this.totalPassengers();return `${total} ${total===1?'traveler':'travelers'}`},increment(key){if(this.totalPassengers()<9&&(key!=='infants'||this.counts.infants<this.counts.adults))this.counts[key]++},decrement(key){const min=key==='adults'?1:0;if(this.counts[key]>min)this.counts[key]--;if(key==='adults'&&this.counts.infants>this.counts.adults)this.counts.infants=this.counts.adults}}}
    </script>
</div>
