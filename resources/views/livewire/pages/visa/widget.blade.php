<div>
    ...
    <section class="vw-hero" x-data="visaWidget()" @click.outside="paxOpen = false">
        <div class="vw-hero__inner">
            ...
            <form class="vw-card" action="{{ route('visa.search') }}" method="POST">
                        ...
                        <div class="vw-pax-dropdown" x-show="paxOpen" x-transition x-cloak>
                            <template x-for="row in passengerRows" :key="row.key">
                                <div class="vw-pax-row">
                                    ...
                                    <div class="vw-pax-controls"><button type="button" @click="decrement(row.key)" :disabled="counts[row.key] <= row.min">âˆ’</button><span x-text="counts[row.key]"></span><button type="button" @click="increment(row.key)" :disabled="totalPassengers() >= 9 || (row.key === 'infants' && counts.infants >= counts.adults)">+</button></div>
                                </div>
                            </template>
                ...
                <div class="vw-fields">
                    ...
                    <label class="vw-field">
                        ...
                        <select name="destination_ref" x-model="destinationRef" @change="sameCountryNotice = false" :disabled="destinationsLoading" required>
                            <option value="" x-text="destinationsLoading ? 'Loading destinations...' : 'Where are you going?'"></option>
                            <optgroup label="Countries" x-show="destinationCountries.length">
                                <template x-for="country in destinationCountries" :key="country.ref">
                                    <option :value="country.ref" x-text="country.name"></option>
                                </template>
                            </optgroup>
                            <optgroup label="Regional visa destinations" x-show="regionalDestinations.length">
                                <template x-for="destination in regionalDestinations" :key="destination.ref">
                                    <option :value="destination.ref" x-text="destination.name"></option>
                                </template>
                            </optgroup>
                        </select>
                        <small x-show="sameCountryNotice" x-cloak role="status">Choose a destination different from your passport nationality.</small>
                        <small x-show="!destinationsLoading && nationalityId && destinationCountries.length === 0 && regionalDestinations.length === 0" x-cloak role="status">No visa destination is currently available for this passport nationality.</small>
                    </label>
                ...
                <div class="vw-card__footer"><p>Final eligibility and visa issuance are determined by the relevant authority.</p><button class="vw-search-btn" type="submit" :disabled="destinationsLoading">Check visa options <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg></button></div>
            </form>
        </div>
    </section>
    ...
    <script>
        function visaWidget() {
            return {
                paxOpen: false,
                nationalityId: @js((string) old('nationality_id', $defaultNationalityId)),
                destinationRef: @js((string) old('destination_ref', '')),
                destinationCountries: @js($destinationCountries->values()),
                regionalDestinations: @js($regionalDestinations->values()),
                destinationsLoading: false,
                sameCountryNotice: false,
                counts: {
                    adults: {{ old('adults', 1) }},
                    children: {{ old('children', 0) }},
                    infants: {{ old('infants', 0) }},
                },
                passengerRows: [
                ...
                async handleNationalityChange() {
                    this.sameCountryNotice = false;
                    if (! this.nationalityId) {
                        this.destinationCountries = [];
                        this.regionalDestinations = [];
                        this.destinationRef = '';
                        return;
                    }
                    ...
                    try {
                        const response = await fetch(@js(route('visa.destinations')) + `?nationality_id=${encodeURIComponent(this.nationalityId)}`, {
                        ...
                        this.destinationCountries = payload.countries || [];
                        this.regionalDestinations = payload.regions || [];

                        const refs = [...this.destinationCountries, ...this.regionalDestinations].map((destination) => destination.ref);

                        if (this.destinationRef && ! refs.includes(this.destinationRef)) {
                            this.sameCountryNotice = this.destinationRef === `country:${this.nationalityId}`;
                            this.destinationRef = '';
                        }
                ...
                increment(key) {
                    if (this.totalPassengers() < 9 && (key !== 'infants' || this.counts.infants < this.counts.adults)) {
                        this.counts[key]++;
                    }
                },
                decrement(key) {
                    const min = key === 'adults' ? 1 : 0;
                    if (this.counts[key] > min) {
                        this.counts[key]--;
                    }
                    if (key === 'adults' && this.counts.infants > this.counts.adults) {
                        this.counts.infants = this.counts.adults;
                    }
                },
            };
        }
