        ...
        <div class="vw-hero__inner">
            ...
            <form class="vw-card" action="{{ route('visa.search') }}" method="POST">
                ...
                <div class="vw-fields">
                    ...
                    <label class="vw-field"><span>Destination</span><select name="destination_id" required><option value="">Where are you going?</option>@foreach($countries as $country)<option value="{{ $country->id }}" @selected(old('destination_id') == $country->id)>{{ $country->name }}</option>@endforeach</select></label>
