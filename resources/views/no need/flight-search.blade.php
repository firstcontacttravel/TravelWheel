...
function flightWidget() {
    return {
        ...
        selectLegAirport(index, field, airport) {
            const val = airport.iata + ' (' + (airport.city || airport.name) + ')';
            const leg = { ...this.multiLegs[index] };
            if (field === 'from') {
                leg.from = val; leg.fromResults = []; leg.fromFocus = false;
            } else {
                leg.to   = val; leg.toResults   = []; leg.toFocus   = false;
            }
            this.multiLegs[index] = leg;
            this.multiLegs = [...this.multiLegs];
        },
        ...
        pickLegDate(index, cell) {
            if (cell.empty || cell.disabled) return;
            this.multiLegs[index].depart = this._fmtDate(new Date(cell.y, cell.m, cell.d));
            this.multiLegs  = [...this.multiLegs];
            this.openLegCal = null;
        },
